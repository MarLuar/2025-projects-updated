#define BLYNK_TEMPLATE_ID "TMPL6P5btfHcz"
#define BLYNK_TEMPLATE_NAME "SmartScore"
#define BLYNK_AUTH_TOKEN "Ui0rXmOAUYnV1JrJlobQ-5T2Fp4UWvoG"
...
void loadFileContentsToZones() {
  ...
  while (file.available() && zoneIndex < 6) {
    ...
    if (zoneIndex >= 0 && zoneIndex < 6) {
      ...
      // Update the display for the zone immediately
      updateZoneDisplay(zoneIndex);
    }
  }
  ...
  Serial.println("File loaded and displays updated.");
}
...
void setup() {
  ...
  delay(5000);  // Ensure all initialization is done

  // Now turn on WiFi
  Serial.println("Turning on WiFi...");
  WiFi.mode(WIFI_STA);  // Enable WiFi in Station mode
  ...
  // Now start Blynk
  Blynk.config(BLYNK_AUTH_TOKEN);  // Use `config()` instead of `begin()` to avoid blocking
  Blynk.connect();
}
