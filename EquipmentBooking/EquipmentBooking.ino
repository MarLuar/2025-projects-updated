...
MFRC522 mfrc522_auth(SS_PIN_2, RST_PIN);
...
const char* equipment[] = {"Projector", "HDMI Cable"}; 
...
std::map<String, bool> lockerStatus;


unsigned long lockOpenStartTime = 0; 
bool isLockOpen = false;            
String currentLockAsset = "";       
...
void setup() {
    ...
    digitalWrite(SOLENOID_PROJECTOR, HIGH); 
    ...
    for (int i = 0; i < 2; i++) { 
        ...
        lockerStatus[equipment[i]] = false; 
    }
...
void handleAuthorization() {
    ...
    if (mfrc522_auth.PICC_IsNewCardPresent() && mfrc522_auth.PICC_ReadCardSerial()) {
        ...
        if (uidString.equalsIgnoreCase("d31a9ca9")) { 
            authorized = true;
...
void checkLockStatus() {
    if (isLockOpen && (millis() - lockOpenStartTime >= 30000)) { 
